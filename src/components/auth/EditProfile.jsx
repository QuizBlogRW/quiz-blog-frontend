...
const EditProfile = () => {
  ...
  const schools = useSelector(
    (state) => state.schools && state.schools.allSchools
  );
  const schoolLevels = useSelector(
    (state) => state.levels && state.levels.schoolLevels
  );
  const levelFaculties = useSelector(
    (state) => state.faculties && state.faculties.levelFaculties
  );
  ...
  useEffect(() => {
    if (profileState.faculty) {
      const faculty = levelFaculties.find(
        (fac) => fac._id === profileState.faculty._id
      );
  ...
  const handleSelectChange = (e, field, fetchAction, list) => {
    ...
    const selectedObject = list
      ? list.find((item) => item._id === value)
      : value;
