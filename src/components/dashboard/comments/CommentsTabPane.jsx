...
const CommentsTabPane = () => {
    ...
    const { allQuestionsComments, paginatedQuestionsComments } = useSelector(state => state.questionsComments)
    const totPages = allQuestionsComments && allQuestionsComments.totalPages
    const paginatedQnsCmts = paginatedQuestionsComments && paginatedQuestionsComments.paginatedQuestionsComments
    ...
    return (
        <TabPane tabId="9">
            ...
            <Row>
                {paginatedQnsCmts?.length > 0 ?
                    <>
                        {(user?.role === 'Admin' || user?.role === 'SuperAdmin') && <PageOf pageNo={pageNo} numberOfPages={numberOfPages} />}
                        ...
                        <Row>
                            <ListGroup>
                                {filteredQuizComments.map(cmnt => <Comment comment={cmnt} key={cmnt._id} />)}
                            </ListGroup>
                        </Row>
                        <Row>
                            <Col sm={12} className="mt-2 comments-card">
                                <Card body>
                                    {paginatedQnsCmts.map((comment, i) => <Comment comment={comment} key={i} />)}
                                    {renderShowAllButton(showAll, setShowAll, allQuizzesComments)}
                                </Card>
                            </Col>
                        </Row>
                        {user?.role !== 'Visitor' && <Pagination pageNo={pageNo} setPageNo={setPageNo} numberOfPages={numberOfPages} />}
                    </> :
