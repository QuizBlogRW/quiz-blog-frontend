...
const CatDropdown = ({ clr }) => {
  ...
  return (
    <div ref={toggleRef} className="profileDropDown position-static">
      <Dropdown isOpen={isOpen} toggle={toggle}>
        <DropdownToggle
          className="bg-transparent border-0 profileDropDown-btn cat-dropdown-toggle"
          aria-haspopup={true}
        ...
        <DropdownMenu
          ...
          role="menu"
          aria-label="Quizzes categories menu"
        >
          <DropdownItem
            header
          ...
          <DropdownItem
            text
            className="text-uppercase d-flex justify-content-end align-items-center"
          >
            <Button
              className="cat-close-btn btn-sm"
              onClick={() => setIsOpen(false)}
          ...
          {categories.allcategories &&
            categories.allcategories.map((category) => (
              <DropdownItem
                text
                key={category._id}
                className="my-1 text-uppercase"
              >
                <Link
                  to={`/category/${category._id}`}
                  className="text-decoration-none cat-dropdown-link"
                  onClick={() => setIsOpen(false)}
