...
export const getLevels = () => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(returnErrors(err && err.response.data, err && err.response.status, 'GET_LEVELS_FAIL'));
...
export const fetchSchoolLevels = (schoolID) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(returnErrors(err.response.data, err.response.status, 'FETCH_SCHOOL_LEVELS_FAIL'));
...
export const createLevel = (newLevel) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(returnErrors(err.response.data, err.response.status, 'CREATE_LEVEL_FAIL'));
...
export const updateLevel = updatedLev => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(returnErrors(err.response.data, err.response.status, 'UPDATE_LEVEL_FAIL'));
...
export const deleteLevel = id => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(returnErrors(err.response.data, err.response.status, 'DELETE_LEVEL_FAIL'));
