...
const imageUploadsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(createImageUpload.fulfilled, (state, action) => {
      ...
      state.imageUploadsByOwner.unshift(action.payload);
      state.isLoading = false;
    });
    builder.addCase(updateImageUpload.fulfilled, (state, action) => {
      state.allImageUploads = state.allImageUploads.map((imageUpload) =>
        imageUpload._id === action.payload._id ? action.payload : imageUpload
      );
      state.imageUploadsByOwner = state.imageUploadsByOwner.map((imageUpload) =>
        imageUpload._id === action.payload._id ? action.payload : imageUpload
      );
      state.isLoading = false;
    });
    builder.addCase(deleteImageUpload.fulfilled, (state, action) => {
      state.allImageUploads = state.allImageUploads.filter(
        (imageUpload) => imageUpload._id !== action.payload._id
      );
      state.imageUploadsByOwner = state.imageUploadsByOwner.filter(
        (imageUpload) => imageUpload._id !== action.payload._id
      );
      state.isLoading = false;
    });
